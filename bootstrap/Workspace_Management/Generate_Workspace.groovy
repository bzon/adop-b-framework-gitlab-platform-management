...
do
		...
		${WORKSPACE}/common/gitlab/create_user.sh -g http://gitlab/gitlab/ -t "${token}" -u "${username}" -p "${username}" -e "${user}" 
done

# create new group			
${WORKSPACE}/common/gitlab/create_group.sh -g http://gitlab/gitlab/ -t "${token}" -w "${WORKSPACE_NAME}"
