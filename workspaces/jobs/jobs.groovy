...
generateProjectJob.with{
    parameters{
        ...
        stringParam("ADMIN_USERS","${INITIAL_ADMIN_USER}@accenture.com","The list of users' email addresses that should be setup initially as admin. They will have full access to all jobs within the project.")
        stringParam("DEVELOPER_USERS","${INITIAL_ADMIN_USER}@accenture.com","The list of users' email addresses that should be setup initially as developers. They will have full access to all non-admin jobs within the project.")
        stringParam("VIEWER_USERS","${INITIAL_ADMIN_USER}@accenture.com","The list of users' email addresses that should be setup initially as viewers. They will have read-only access to all non-admin jobs within the project.")
    }
    label("ldap")
...
gid="$(curl --header "PRIVATE-TOKEN: $token" "http://gitlab/gitlab/api/v3/groups/${WORKSPACE_NAME}" | python -c "import json,sys;obj=json.load(sys.stdin);print obj['id'];")"
				
# create new project				
${WORKSPACE}/common/gitlab/create_project.sh -g http://gitlab/gitlab/ -t "${token}" -w "${gid}" -p "${PROJECT_NAME}"
				
# get project id
pid="$(curl --header "PRIVATE-TOKEN: $token" "http://gitlab/gitlab/api/v3/projects/${WORKSPACE_NAME}%2F${PROJECT_NAME}" | python -c "import json,sys;obj=json.load(sys.stdin);print obj['id'];")"
				
# add the users to the project as owners
for owner in $ADMIN_USERS
do
		ownername=$(echo ${owner} | cut -d'@' -f1)
		uid="$(curl --header "PRIVATE-TOKEN: $token" "http://gitlab/gitlab/api/v3/users?username=${ownername}" | python -c "import json,sys;obj=json.load(sys.stdin);print obj[0]['id'];")"
...
do
		...
		uid="$(curl --header "PRIVATE-TOKEN: $token" "http://gitlab/gitlab/api/v3/users?username=${developername}" | python -c "import json,sys;obj=json.load(sys.stdin);print obj[0]['id'];")"
...
do
		...
		uid="$(curl --header "PRIVATE-TOKEN: $token" "http://gitlab/gitlab/api/v3/users?username=${guestname}" | python -c "import json,sys;obj=json.load(sys.stdin);print obj[0]['id'];")"
